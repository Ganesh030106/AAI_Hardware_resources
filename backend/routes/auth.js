...
router.post('/superadmin/add-user', async (req, res) => {
    try {
        ...
        // Only allow Employee, Admin, or Technician
        if (!['Employee', 'Admin', 'Technician'].includes(role)) {
            return res.status(400).json({ message: "Role must be 'Employee', 'Admin', or 'Technician'" });
        }
...
// POST /technician-login - Technician login only
