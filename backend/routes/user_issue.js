    ...
    try {
        ...
        const enriched = await Promise.all(
            requests.map(async (req) => {
                ...
                if (req.technician_status && req.technician_status !== "Unassigned") {
                    const techId = req.assigned_to || req.technician_id;
                    if (techId) {
                        const tech = await User.findOne({ emp_id: techId }).lean();
                        technicianName = tech ? tech.name : req.technician_status;
                    } else {
                        technicianName = req.technician_status;
                    }
                }
...
router.get("/:id", async (req, res) => {
    try {
        ...
        let technicianName = "Unassigned";
        if (request.technician_status && request.technician_status !== "Unassigned") {
            const techId = request.assigned_to || request.technician_id;
            if (techId) {
                const tech = await User.findOne({ emp_id: techId }).lean();
                technicianName = tech ? tech.name : request.technician_status;
            } else {
                technicianName = request.technician_status;
            }
        }
